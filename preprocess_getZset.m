...
function [peak]=quad_pk(x,y)
	...
	y1 = y(args);
	x1 = x(args);
	% Calculation of the coeficients of the quadratic polinomy
	c = quadfit(x1,y1);
	% Term of first derivate of the second order polinomic equation
	peak = -c(2)/(2*c(1));
end

function c=quadfit(x,y)
% Calculate the coeficients of the quadratic polinomi, with the form ax.^2 + b.*x + c
% INPUT:
%	x, y : points of the curve to fite
% OUTPUT:
%	c: coeficiens of the polinomy (a, b, c), where a=c(1), and b=c(2)
	X=[x.^2 x ones(size(x))];
	c=X\y;
end
